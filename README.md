...
**但凡能选 就不要写java 浪费时间浪费人生 写好py和go足以**
======

**对于java只需停留在相通的架构设计的理解层面即可 不需要也没必要再去写它**        
**如果要写java就一定要使用这2个工具 = idea + gradle7/maven3 来构建项目 = 可以省去各种麻烦(踩TM无数坑了浪费时间) !!!**              
**一个套壳绕开写java的方式 = 使用JNA调用golang编译的.so动态库 !!!**     
